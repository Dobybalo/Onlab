...
class Batcher(object):
  ...
  def _FillInputQueue(self):
    ...
    while True:
      ...
      for i in xrange(min(self._max_article_sentences,
        ...
        enc_inputs += data.GetWordIds(article_sentences[i], self._vocab) # encoder input = article sentences converted
      for i in xrange(min(self._max_abstract_sentences,
                          len(abstract_sentences))):
        dec_inputs += data.GetWordIds(abstract_sentences[i], self._vocab) # decoder inputs = abstract sentences converted
  ...
  def _TextGenerator(self, example_gen):
    ...
    while True:
      ...
      try:
        article_text = e[0]  #article_text = self._GetExFeatureText(e, self._article_key)
        abstract_text = e[1] #abstract_text = self._GetExFeatureText(e, self._abstract_key)
      except ValueError:
