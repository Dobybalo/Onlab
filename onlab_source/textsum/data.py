...
def ExampleGen(data_path, num_epochs=None):
  ...
  while True:
    ...
    for f in filelist:
      #reader = open(f, 'rb')
      csvReader = csv.reader(io.open(f, mode='rU', encoding='utf-8')) #csvReader = csv.reader(codecs.open(f, 'rU', 'utf-8'))
      firstLine = True
      for row in csvReader:
        ...
        # meg el kell tavolitani a newline es a "\xa0" karaktereket a szovegbol - \n, \xa0
        article_text, abstract_text = row[3], row[1]
        article_text = article_text.replace('\n', ' ').replace('\xa0', ' ')
        abstract_text = abstract_text.replace('\n', ' ').replace('\xa0', ' ')
            
        # Itt csinaljuk meg a mondatokra bontast
        # hogy ne kelljen modositani a listat, uj valtozoban taroljuk el az uj szoveget
        article_text = addSentenceTokens(article_text)
        abstract_text = addSentenceTokens(abstract_text)
        ...
        # meg kell oldanunk itt a beolvasast ugy, hogy yield-elni lehessen az (article, abstract) tuple-oket#
...
def addSentenceTokens(text):
    new_text = ""
    sentences = re.split(r'(?<!\w\.\w.)(?<![A-Z][a-z]\.)(?<=\.|\?)\s', text) # lehet hogy nem kell elejere a r ?
    for sentence in sentences:
        addition = "<s>" + sentence + "</s>"
        new_text += addition
    return new_text        
...
def SnippetGen(text, start_tok, end_tok, inclusive=True):
  ...
  while True:
    try:
      start_p = text.index(start_tok, cur) # next occurence of start token - cursor! 
      end_p = text.index(end_tok, start_p + 1) # next occurence of end token 
      cur = end_p + len(end_tok) # cursor goes over end token
      if inclusive:
